...
def visualize_test_load_curve(resample_test_df):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=resample_test_df.index, y=resample_test_df['mains'], name='Load Curve'))
    fig.update_layout(showlegend=True, title='resampled test load curve')
...
def visualize_test_train_load_curve(resample_train_df, resample_test_df):
    fig = go.Figure()
